...
**Custom OAuth redirect page**: The custom flow uses a static redirect page (default: `https://smartsync.takezo.dev/callback`) instead of the auth server. This page is a single HTML file with no external scripts, stylesheets, or dependencies — it only reads the authorization code from the URL and redirects to `obsidian://smart-sync-auth?code=...&state=...`. The authorization code is protected by PKCE (S256): even if intercepted, it cannot be exchanged for tokens without the `code_verifier` held only by the plugin. Users can host their own redirect page and configure its URL in the settings.
