...
Custom OAuth backend (`type: "googledrive-custom"`). Uses `GoogleAuthDirect` (direct PKCE flow with user-provided credentials). The user owns the OAuth client and provides their own client ID, client secret, scope, and redirect URI. Extends `GoogleDriveBackendData` with `customClientId`, `customClientSecret`, `customScope`, and `customRedirectUri`. Data is stored in `settings.backendData["googledrive-custom"]`.
